...
const s3Storage = multerS3({
  ...
  key: function (req, file, cb) {
    ...
    const fileName = `uploads/image-${timestamp}-${randomSuffix}${path.extname(file.originalname)}`;
    cb(null, fileName);
  },
});
const storage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    const timestamp = Date.now();
    const randomSuffix = Math.floor(Math.random() * 10000);
    const fileName = `image-${timestamp}-${randomSuffix}${path.extname(file.originalname)}`;
    cb(null, fileName);
  },
});
