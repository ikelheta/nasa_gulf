...
import consultantServ from "../../../consultant/v1/admin/consultant.service";
...
export async function loginAdmin(req : Request, res : Response) {
    ...
    const admin = await consultantServ.findOne({ where: { email }})
    if(!admin){
...
export async function forgetPassword(
  ...
  const admin: any = await consultantServ.findOne({
...
export async function otpValidation(
  ...
  const admin = await consultantServ.findOne({
    where: {
      email,
    },
    attributes: ["id"],
...
export async function updatePassword(
  ...
  const updatedPassword: any = await consultantServ.update(
    {
