...
class Consultant extends Model<
  InferAttributes<Consultant>, // Attributes available on Admin instances
...
Consultant.beforeCreate(async (user: any, options) => {
  const hashedPassword = await user.hashPassword(user.password);
  user.password = hashedPassword;
});

Consultant.beforeUpdate(async (user: any, options) => {
